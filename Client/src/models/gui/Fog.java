...
class Fog {
    ...
    private void moveFog() {
        AnimationTimer anim = new AnimationTimer() {
            ...
            private final double velocity = UIConstants.FOG_VELOCITY;
            private double currentX = UIConstants.SCENE_WIDTH;
            private double currentY = UIConstants.SCENE_HEIGHT - UIConstants.FOREGROUND_HEIGHT * 0.75;
            ...
            public void handle(long now) {
                if (now - lastTimeStarted > DURATION) {
                    lastTimeStarted = now;
                    currentX = UIConstants.SCENE_WIDTH;
                } else {
                    currentX -= velocity;
                }
    ...
    private Circle createFogElement() {
        double centerX = UIConstants.FOG_CIRCLE_RADIUS + random.nextInt((int) (width - UIConstants.FOG_CIRCLE_RADIUS * 2));
        double centerY = UIConstants.FOG_CIRCLE_RADIUS + random.nextInt((int) (height - UIConstants.FOG_CIRCLE_RADIUS * 2));
        int radius = UIConstants.FOG_CIRCLE_RADIUS * 4 / 5 + random.nextInt(UIConstants.FOG_CIRCLE_RADIUS / 2);
